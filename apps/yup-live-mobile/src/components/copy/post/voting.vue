  ...
  <div class="flex gap-4">
    <!-- <o-tooltip v-model:active="showUp" :triggers="['hover']" :autoClose="true" :multiline="true"> -->
      ...
      <ThumbsUp
        ...
        :class="`opacity-80 w-5 cursor-pointer inline-block mr-1 thumb ${doingVote ? 'blink' : ''}`"
        :isSolid="refHasVote && lastVote"
        @click="doVote(true)"
      />
      <span v-if="positiveWeightRef" class="opacity-80 mr-1 text-[0.85rem]" v-html="formatNumber(positiveWeightRef, 2)"></span>
    </div>
    <!-- </o-tooltip>
    <o-tooltip v-model:active="showDown" :triggers="['hover']" :autoClose="true" :multiline="true"> -->
      ...
      <ThumbsDown
        ...
        @click="doVote(false)"
      />
      <span v-if="negativeWeightRef" class="opacity-80 mr-1 text-[0.85rem]" v-html="formatNumber(negativeWeightRef, 2)"></span>
    </div>
    <!-- </o-tooltip> -->
  </div>
</template>
