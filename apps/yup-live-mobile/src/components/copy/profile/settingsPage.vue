<template>
  ...
  <template v-else>
    <h2 class="text-[1.3rem] my-4 uppercase">Settings</h2>
      ...
      >
        ...
        >
          ...
          <ImagePreview
            ...
            :imgClass="`rounded-b-full rounded-t-full ${
              token.symbol === 'WETH' ? 'h-15 w-15' : 'h-13 w-13'
            }`"
            :noPreviewClass="`rounded-b-full rounded-t-full ${
              token.symbol === 'WETH' ? 'h-15 w-15' : 'h-13 w-13'
            }`"
      ...
      if (!props.accountEVMAddr) {
        ...
        getProfileWallet().then((r) => {
          ...
          if (r?.nfts.length > 0 && r?.tokens.length > 0 && r?.poaps.length > 0) {
            refTokensPoly.value =
              r?.tokens.filter((e: { chain: string }) => e.chain === "polygon") ?? [];
            refTokensEth.value =
              r?.tokens.filter((e: { chain: string }) => e.chain === "ethereum") ?? [];
            refNftsPoly.value =
              r?.nfts
                .filter((e: { chain: string }) => e.chain === "polygon")
                .filter((e: IProfileNFT) => e?.imageURI) ?? [];
            refNftsEth.value =
              r?.nfts.filter((e: { chain: string }) => e.chain === "ethereum") ?? [];
            POAPS.value = r?.poaps ?? [];
