            ...
            <li v-for="voter of voters" :key="voter.a._id" class="p-4">
              <div class="flex p-2">
                ...
                <ion-button
                  ...
                  :router-link="`/tabs/account/${voter.a.username}`"
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const blockPost = async () => {
      try {
        addBlockedPost(props.postId);
        await wait(150);
        store.deletePost = props.postId;
        ctx.emit("deletedvote");
        stackAlertSuccess("Entry successfully blocked.");
      } catch {
        stackAlertError("There was an error with the block, please try again later.");
      }
    };
