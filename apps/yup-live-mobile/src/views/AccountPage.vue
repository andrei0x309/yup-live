<template>
  ...
  <div class="page lg:max-width-90 md:max-width-60 py-2 mx-auto mb-8">
    ...
    <div v-if="!apiError" class="bg-color table-list profile w-full mb-4 flex flex-col">
      ...
      <InfScroll
        v-if="[accountPages[0], accountPages[1]].includes(currentAccountPage)&& postLoaded"
...
export default defineComponent({
  ...
  setup() {
    ...
    const getCreatedFeedPosts = async (start = 0) => {
      const res = await fetch(`${API_BASE}/profile/posts/${userData.value.evmAddress}/?start=${start}&limit=10`)
      const data = await res.json()
      return data.posts
    }
