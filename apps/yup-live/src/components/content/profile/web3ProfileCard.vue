<template>
  ...
  <figcaption>
    ...
    <div v-if="web3Profile?.yup?.handle">
      <ProfileYupIcon class="mr-2 w-4 inline-block" /> {{ web3Profile.yup.handle }} <router-link :to="`/profile/${web3Profile.yup.handle}`" class="view-btn" >VIEW</router-link>
    </div>
    ...
    <div v-if="web3Profile?.farcaster?.handle">
      <ProfileFarcasterIcon class="mr-2 w-4 inline-block" /> {{ web3Profile.farcaster.handle }}
    </div>
