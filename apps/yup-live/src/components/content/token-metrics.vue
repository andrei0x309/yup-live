  ...
  <div class="bg-color token-metrics w-full">
    <div>
      ...
      <template v-else>
        ...
        <sup
          :class="`p-1 font-06 ${
            dataGecko.value.priceChangeRaw < 0 ? 'price-red' : 'price-green'
          }`"
        >
          {{ dataGecko.value.priceChange }}%
        </sup>
        <span class="block font-088 faded-darker mt-2">{{
          `Mcap: $${dataSupply.value.mcap}`
        }}</span>
      </template>
    </div>
    <div>
      ...
      <template v-else>
        ...
        <span class="block font-088 faded-darker mt-2">{{
          `/ ${dataSupply.value.maxSupply} YUP`
        }}</span>
      </template>
    </div>
    <div>
      ...
      <template v-else>
        ...
        <span class="block font-088 faded-darker mt-2"
          >This month: {{ dataActionCount.value.actCountMonth }}</span
        >
      </template>
    </div>
    <div>
      ...
      <template v-else>
        ...
        <span class="block font-088 faded-darker mt-2">{{
          `$${dataEmissions.value.emissionsValue}`
        }}</span>
      </template>
    </div>
  </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    type DataSupply = Ref<
      ComputedRef<{ supply: string; mcap: string; maxSupply: string }>
    >;
    ...
    const currentDayEmission = async () => {
      ...
      const yearData = await import(
        /* @vite-ignore */ `/emissions/years/${today.getFullYear()}.js` /* @vite-ignore */
      );
      const dateString = `${today.getFullYear()}-${(
        "0" + String(today.getMonth() + 1)
      ).slice(-2)}-${("0" + String(today.getDate())).slice(-2)}`;
    ...
    const getGeckoData = async () => {
      const req = await fetch("https://yup-token-cache-hono.deno.dev/token/yup", {
        headers: {
          Authorization:
            (Math.random() / 20.2).toString(36).substring(2) + Date.now().toString(36),
        },
      });
      if (!req.ok) {
        throw new Error(`Request failed with status ${req.status}`);
      ...
      const gKP = getGeckoData().then((gecko: any) => {
        dataGecko.value = computed(() => {
          return {
            ...
            priceChange: Math.abs(gecko.market_data.price_change_percentage_24h).toFixed(
              2
            ),
          };
        });
