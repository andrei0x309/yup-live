<template>
  ...
  <div class="bg-color table-list sellers w-full mb-4 m-auto">
    ...
    >
      ...
      <o-table-column v-slot="props" field="accounts" label="USER">
        <div class="inline">
          ...
          <a
            v-else-if="props.row.accounts !== 'unknown'"
            :href="`/profile/${props.row.accounts[0]}`"
            rel="nofollow"
            target="_blank"
          >
        ...
        >
          <template #content>
            <div class="inline" style="font-size: 0.81rem">
              ...
              <p v-for="addr of props.row.related_addresses.items" :key="addr">
                {{ addr }}
              </p>
            </div>
          </template>
    ...
    <div class="pag">
      <p
        v-if="lastBlockProcesed && blockTime"
        class="time"
        style="top: 1rem; right: 20rem"
      >
        <QuickSwapIcon class="sIcon" style="width: 1.14rem" /> QuickSwap sells get
        processed from time to time, last block processed:
      ...
      exportFile(
        ...
        convertToCSV(
          withHeader.map((v) => {
            ...
            v.related_accounts_full = (v.related_accounts_full as string).replace(
              /,/gm,
              " "
            );
            return {
