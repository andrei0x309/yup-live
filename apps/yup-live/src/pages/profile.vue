<template>
  <div class="page lg:max-width-90 md:max-width-60 py-2 mx-auto mb-8">
    <div class="bg-color flex flex-col">
      ...
      <ProfileMenu :isOwnAccount="isOwnAccount" :currentMenuTab="currentMenuTab" @change="menuChange" />
    </template>
    ...
    <div v-if="!apiError" class="bg-color table-list profile w-full mb-4 flex flex-col">
      ...
      />
      <SettingsPage v-if="currentMenuTab === MENU_BUTTONS.settings" :userData="userData" />
...
<script lang="ts">
import { onMounted, defineComponent, reactive, computed, onUnmounted, Ref, ref, watch, defineAsyncComponent } from 'vue'
...
export default defineComponent({
  ...
  components: {
    ...
    FollowersPage,
    WalletPage,
    SettingsPage: defineAsyncComponent(() => import('@/components/content/profile/settingsPage.vue'))
  },
  setup() {
    ...
    const isOwnAccount = ref(store?.isLoggedIn && store?.userData.account === userId.value)
    ...
    const getActionUsage = (userId: string) => {
      createActionUsage(userId, userData.value.balanceNum).then((d) => {
        if (!d.error) {
          siteData.title = `Yup Profile - ${userData.value?.username ?? userData.value?._id ?? ''}`
