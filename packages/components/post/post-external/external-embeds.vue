<template>
<component v-for="comEm of compEms" :is="comEm.component" :source="comEm.source" :type="comEm.type" :linkPreview="comEm.linkPreview" :key="comEm.source"/>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    type EmbedComp = {component: ReturnType<typeof defineComponent>, source: string, type?: String, linkPreview?: linkPreviewTypeEx }
      ...
      onBeforeMount(async () => {
        ...
        for (const em of props.embeds) {
            if(embedsComp[em.type]) {
              localCompEms.push({component: embedsComp[em.type], source: em.url, type: em.type, linkPreview: em.linkPreview})
                continue
            }
