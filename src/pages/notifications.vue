<template>
  ...
  >
   <template v-if="notification.action === 'vote'">
        ...
        <div class="flex">
        <p class="ml-3 flex items-center pb-2"><b class="mr-2 opacity-60">by</b> <router-link :to="`/profile/${notification.voter}`">{{ notification.voter}}</router-link></p>
        </div>
        <p class="ml-3 mt-3 flex items-center w-full"><b class="mr-2 opacity-60" style=" writing-mode: vertical-rl;text-orientation: upright;">URL</b><span style="font-size:0.78rem"><router-link :to="`/p/${notification?.post?.postid}`">{{ notification?.post?.url }}</router-link></span></p>
        ...
        <div class="flex w-full">
        <div class="flex flex-col">
        <p class="ml-3 mt-1 pb-4 flex items-center">
        You 
        </p>
        <ImagePreview
        :source="notification.image" 
        ...
        <p class="ml-3 mt-3 flex items-center"><b class="mr-2 opacity-60">{{notification?.quantity ?? 'unknown'}}</b> YUP</p>
        <p class="ml-3 flex items-center mt-4 self-end"><ClockIcon class="h4 w-4 mr-1" />{{ timeAgo(notification.createdAt) }}</p>
        </div>
        </div>
        </template>
  ...
  >
    ...
    <div class="flex w-full">
    <div class="flex flex-col">
    <p class="mt-4 ml-4 pb-4 flex items-center">
    You 
    </p>
    <ImagePreview
    :source="notification.image" 
    ...
    <p class="ml-3 flex items-center pb-2 mt-4"><b class="mr-2 opacity-60">were alocated a future reward of:</b></p>
     ...
     <p class="ml-6 flex items-center mt-6"><b class="mr-2 opacity-60">{{notification?.quantity ?? 'unknown'}}</b> YUP</p>
     <p class="ml-3 flex items-center mt-4 self-end"><ClockIcon class="h4 w-4 mr-1" />{{ timeAgo(notification.createdAt) }}</p>
    </div>
    </div>
    </div>
...
import ThumbsDown from "@/components/content/icons/thumbsDown.vue"
import ThumbsUp from "@/components/content/icons/thumbsUp.vue"
...
export default defineComponent({
  ...
  setup() {
    ...
    const getNotifications = async ({
      ...
      limit,
      skip,
    } = { type: 'all', limit: '10', skip: '0'  } as { type: string, limit?: string, skip?: string }) => {
      let req 
      
      if (type === 'all') {
      req = await fetch(`${API_BASE}/notifications/${userId}?sky=${skip}&limit=${limit}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json;charset=utf-8",
        },
        ...
        req = await fetch(`${API_BASE}/notifications/${userId}?sky=${skip}&limit=${limit}&type=vote`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json;charset=utf-8",
        },
        ...
        req = await fetch(`${API_BASE}/notifications/${userId}?sky=${skip}&limit=${limit}&type=reward`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json;charset=utf-8",
        },
    ...
    if(activeTab.value==='0') {
        notifications.value = (await getNotifications()).reverse()
       } else if(activeTab.value==='1') {
        notifications.value = (await getNotifications({type: 'vote'})).reverse()
       } else if(activeTab.value==='2') {
        notifications.value = (await getNotifications({type: 'reward'})).reverse()
       }
       console.log('dsss')
    }
